...
:: (Repo copy: this .bat is NOT next to mpv.exe, so detect it instead of
::  assuming the parent folder. Checks common install dirs, then PATH.)
set "mpv_path="
for %%P in ("C:\Program Files\MPV Player\mpv.exe" "C:\Program Files\mpv\mpv.exe") do if exist "%%~P" set "mpv_path=%%~P"
if not defined mpv_path for %%I in (mpv.exe) do if not defined mpv_path set "mpv_path=%%~$PATH:I"
if not defined mpv_path call :die "mpv.exe not found - install mpv first"
...
call :add_type "image/avif"     "video" "AVIF Image"        ".avif"
call :add_type ""               "audio" "DSD Audio"         ".dsf"
call :add_type ""               "audio" "DSDIFF Audio"      ".dff"
call :add_type "audio/musepack" "audio" "Musepack Audio"    ".mpc" ".mp+"
call :add_type "audio/x-caf"    "audio" "CoreAudio"         ".caf"
call :add_type ""               "audio" "Wave64 Audio"      ".w64"
call :add_type ""               "audio" "AC-4 Audio"        ".ac4"
